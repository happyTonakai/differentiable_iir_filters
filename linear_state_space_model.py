...
class LinearStateSpaceModel(Module):
    def __init__(self, num_states):
        ...
        self.state_and_input_to_output_layer = Linear(
            num_states + 1, 1, bias=False)
    ...
    def forward(self, input, initial_states=None):
        ...
        predicted_output_sequence = torch.zeros(
            batch_size, sequence_length, 1).to(device)

        states_sequence = torch.zeros(
            (batch_size, sequence_length, self.num_states)).to(device)

        for i in range(sequence_length - 1):
            hidden = self.cell(input[:, i, :], hidden)
            states_sequence[:, i+1, :] = hidden[:, :]

        predicted_output_sequence = self.state_and_input_to_output_layer(
            torch.cat([input, states_sequence], -1))
...
class LinearStateSpaceCell(Module):
    ...
    def forward(self, input, in_states):
        state_output = self.state_to_state_layer(
            in_states) + self.input_to_state_layer(input)
        return state_output
